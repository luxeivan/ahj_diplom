import {
  merge, fromEvent, debounceTime, distinctUntilChanged, EMPTY,
} from 'rxjs';
import {
  map, switchMap, tap, catchError, filter,
} from 'rxjs/operators';
...
export default class Chat {
  ...
  init() {
    ...
    ).pipe(map(() => this.inputSearch.value), switchMap((text) => this.getTenMessages(0, text)))
      .subscribe(
        (res) => {
    ...
    // подписка на события drag&drop
    fromEvent(this.chat, 'dragleave').subscribe(() => {
    ...
    fromEvent(this.chatListArea, 'scroll')
      .pipe(
        filter(() => this.chatListArea.scrollTop === 0),
        map(() => document.getElementsByClassName('chat__item').length),
        switchMap((first) => this.getTenMessages(first, this.inputSearch.value)),
  ...
  stringToLink(text) {
    this.one = 'one';
    const matched = text.match(/http[s]?:\/\/\S*/g);
    let result = '';
